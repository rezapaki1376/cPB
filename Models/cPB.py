...
class cPB:
    ...
    def __init__(
      ...
      if pretrain_model_addr!='':
        self.model = ModifiedRNN(pretrain_model_addr=pretrain_model_addr,hidden_size=self.hidden_size,base_model=base_model,seq_len=seq_len,mask_weights=mask_weights,mask_init=mask_init,input_size=self.input_size)
    ...
    def predict_many_1(self,x,y,task_number,mask_selection=False):
      self.model=self.rebuild_model(task_number)
      x = np.array(x)
      y = list(y)
      x, y, _ = self._load_batch(x, y)
      ...
      if mask_selection==False:
        self.performance[f'task_{task_number+1}']['acc'].append(acc)
        self.performance[f'task_{task_number+1}']['kappa'].append(kappa)
      #return acc, kappa
